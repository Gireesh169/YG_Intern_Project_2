...
const CourseDetail = () => {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        setLoading(true);
        const response = await supabaseService.getCourseDetails(courseName);
        dispatch(setSubjectData(response.subject));
        dispatch(setModulesData(response.modules));
        dispatch(setTotalModules(response.totalModules));
      } catch (err) {
  ...
  const fetchCourseDetails_new = async () => {
    try {
      const courseId = "abc123"; // replace with dynamic courseId
      const { course, quizzes } = await supabaseService.getCourseDetails(courseId);
  ...
  useEffect(() => {
    const fetchAttemptedSubModules = async () => {
      try {
        console.log("Fetching attempted with:", { googleId, courseId });
        const response = await supabaseService.getAttemptedSubModules(googleId, courseId);
        console.log("Attempted response:", response);
        setAttemptedList(response.attemptedSubmodules || []);
      } catch (err) {
  ...
  const resetQuizData = async (googleId, subModuleId) => {
    try {
      await supabaseService.resetQuiz(googleId, subModuleId);
      toast.success("Quiz reset successfully!");

      // Refresh the attempted list to update UI
      if (googleId && courseId) {
        const response = await supabaseService.getAttemptedSubModules(googleId, courseId);
        setAttemptedList(response.attemptedSubmodules || []);
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0f0f1a] pt-4 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-[#0f0f1a] pt-4 pb-12">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-purple-900 hover:text-purple-600 mb-6 transition-colors">
          <ArrowLeft size={20} />
